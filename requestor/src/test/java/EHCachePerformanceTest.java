...
@Disabled("This is a performance test run manually.")
public class EHCachePerformanceTest {

    private int idCounter = 0;
    private StatisticsService statService;
    private PersistentCacheManager cacheManager;
    private Cache<String, CacheValue> diskCache;

    @BeforeEach
    void setUp() {
        statService = new DefaultStatisticsService();
        ...
        diskCache = cacheManager.getCache("mixedCache", String.class, CacheValue.class);
    }
    ...
    void mainEhTest() {
        ...
        for (int i = 0; i < 1; i++) { //reading all values once so values are put into memory, to test memory read time
            for (int j = 0; j < keyCount; j++) {
                CacheValue foundItem = diskCache.get(keys[j]);
            }
        }
        long startReadTime = System.nanoTime();
        for (int i = 0; i < keyCount; i++) {
            CacheValue foundItem = diskCache.get(keys[i]);
            if (foundItem != null) {
                foundCount++;
            }
